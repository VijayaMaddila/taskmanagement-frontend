...
function Projects() {
  ...
  const [deleteTarget, setDeleteTarget] = useState(null); // project id
  ...
  // Step-2: collect members locally, then submit everything together
  const [modalStep,      setModalStep]      = useState('form');    // 'form' | 'add-team'
  const [allUsers,       setAllUsers]       = useState([]);
  const [pendingMembers, setPendingMembers] = useState([]);         // collected before submit
  const [addTab,         setAddTab]         = useState('direct');   // 'direct' | 'invite'
  const [memberForm,     setMemberForm]     = useState({ type: 'direct', userId: '', email: '', role: 'DEVELOPER' });
  ...
  // For each project, fetch team members via GET /api/projects/{id}/members
        ...
        try {
          ...
          return {
            ...
            members: members.map(m => ({
              ...m,
              username: m.user?.username || m.user?.name || m.username || m.name || '?',
            })),
          };
        } catch {
  ...
  // Add a member to the local pending list (step 2, no API call yet)
  const handleAddMember = (e) => {
    e.preventDefault();
    if (memberForm.type === 'direct' && !memberForm.userId) { setMemberFormError('Please select a user.'); return; }
    if (memberForm.type === 'invite' && !memberForm.email.trim()) { setMemberFormError('Email is required.'); return; }

    const member = memberForm.type === 'direct'
      ? { userId: Number(memberForm.userId), role: memberForm.role,
          _display: allUsers.find(u => String(u.id) === String(memberForm.userId))?.username || `User #${memberForm.userId}` }
      : { email: memberForm.email.trim(), role: memberForm.role,
          _display: memberForm.email.trim() };

    setPendingMembers(prev => [...prev, member]);
    setMemberForm({ type: memberForm.type, userId: '', email: '', role: 'DEVELOPER' });
    setMemberFormError('');
  };

  // Final submit: create team → add members → create project → send invites
  const handleCreateProject = async () => {
    setCreating(true);
    try {
      // Auto-include the current form entry if filled but not added to list
  ...
  const openModal = (project = null) => {
    ...
    if (project) {
      ...
      setFormData({ name: "", description: "", startDate: "", endDate: "", slackWebhookUrl: "", teamName: "" });
    }
  ...
  const sortedProjects = [...filteredProjects].sort((a, b) => {
    ...
    if (sortBy === "progress") {
      const aProgress = (() => { const t = Array.isArray(a.tasks) ? a.tasks : []; return t.length ? t.filter(x => x.status === "DONE").length / t.length : 0; })();
      const bProgress = (() => { const t = Array.isArray(b.tasks) ? b.tasks : []; return t.length ? t.filter(x => x.status === "DONE").length / t.length : 0; })();
      ...
      <div className="projects-body">
        <div className="projects-header">
          ...
          <div className="pj-header-left">
            <h1>Projects</h1>
            <p>
              {sortedProjects.length}{search ? ` of ${projects.length}` : ""} project{sortedProjects.length !== 1 ? "s" : ""}
            </p>
          </div>

          {/* Center: search */}
          <div className="pj-search-wrap">
            <svg className="pj-search-icon" width="14" height="14" viewBox="0 0 16 16" fill="none">
        ...
        {!loading && sortedProjects.length > 0 && view === "board" && (
          <div className="projects-grid">
            {sortedProjects.map((project) => {
              ...
              const cardAccent = AVATAR_COLORS[project.id % AVATAR_COLORS.length] || '#6366f1';

              return (
                <div key={project.id} className="project-card" onClick={() => navigate(`/board?project=${project.id}`)} style={{ cursor: 'pointer' }}>
                  <div className="pj-card-accent" style={{ background: `linear-gradient(90deg, ${cardAccent}, ${cardAccent}88)` }} />
                  <div className="pj-card-body">
                  <div className="project-header">
                    <div className="pj-card-title-wrap">
                      <div className="pj-card-avatar" style={{ background: cardAccent }}>
                    ...
                    {canManage && (
                      <div className="project-actions">
                        ...
                        <button onClick={(e) => { e.stopPropagation(); setDeleteTarget(project.id); }} className="btn-icon btn-delete">
                          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor">
                  ...
                  <div className="project-members">
                    <div className="pj-members-header">
                      ...
                      <button
                        ...
                        onClick={(e) => { e.stopPropagation(); navigate(`/team?project=${project.id}`); }}
                      >
                        View All →
                      </button>
                    </div>
                    {(() => {
                      ...
                      return (
                        <div className="pj-avatar-stack">
                          ...
                          {nonAdmins.length > 5 && (
                            <div className="pj-stack-avatar pj-stack-more">+{nonAdmins.length - 5}</div>
                          )}
                        </div>
                      );
                    })()}
                  </div>
                  </div>{/* end pj-card-body */}
                </div>
              );
            })}
          </div>
        ...
        {!loading && sortedProjects.length > 0 && view === "list" && (
          <div className="pj-list-view">
            ...
            {sortedProjects.map((project) => {
              ...
              return (
                <div key={project.id} className="pj-list-row" onClick={() => navigate(`/board?project=${project.id}`)} style={{ cursor: 'pointer' }}>
                  <div className="pj-list-name">
                    <div className="pj-list-avatar" style={{ background: AVATAR_COLORS[project.id % AVATAR_COLORS.length] || '#6366f1' }}>
                      {project.name[0].toUpperCase()}
                    </div>
          ...
          <div className="modal-content" onClick={(e) => e.stopPropagation()}>
            ...
            {!editingProject && (
              <div className="pj-steps">
                <div className={`pj-step ${modalStep === 'form' ? 'pj-step--active' : 'pj-step--done'}`}>
                  <span className="pj-step-num">{modalStep === 'form' ? '1' : '✓'}</span>
                  <span>Project Details</span>
                </div>
                <div className="pj-step-connector" />
                <div className={`pj-step ${modalStep === 'add-team' ? 'pj-step--active' : ''}`}>
              ...
              <form onSubmit={handleSubmit}>
                ...
                <div className="form-group">
                  <label htmlFor="slack-webhook">
                    Slack Webhook URL <span className="form-optional">(optional)</span>
                  </label>
                  <input
                    id="slack-webhook"
            ...
            {/* ── Step 2: Collect team members (no API call until Create) ── */}
            {modalStep === 'add-team' && (
              <div className="pj-add-team">
                ...
                <div className="modal-actions pj-team-actions">
                  <button type="button" className="btn-secondary" onClick={() => setModalStep('form')}>
                    ← Back
                  </button>
                  <button type="button" className="btn-secondary" onClick={closeModal}>
                    Skip
                  </button>
                  <button type="button" className="btn-primary" onClick={handleCreateProject} disabled={creating}>
                    {creating ? 'Creating...' : `Create Project${pendingMembers.length > 0 ? ` with ${pendingMembers.length} member${pendingMembers.length > 1 ? 's' : ''}` : ''}`}
                  </button>
                </div>
              </div>
            )}
